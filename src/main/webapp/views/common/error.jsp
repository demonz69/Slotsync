...
<%
    // Collect error details — works both as an error page and with request attributes
    ...
    if (errorMsg   == null || errorMsg.isEmpty()) {
        switch (statusCode) {
            case 400: errorMsg = "Bad request — the server couldn't understand your request."; break;
    ...
    // Stack trace (only shown in development — remove in production)
    String stackTrace = "";
    ...
    String icon  = "⚠";
    if (statusCode == 404) { title = "Page Not Found";      icon = "🔍"; }
    if (statusCode == 403) { title = "Access Denied";       icon = "🔒"; }
    if (statusCode == 400) { title = "Bad Request";         icon = "⚡"; }
%>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title><%= statusCode %> — <%= title %> | SlotSync</title>
